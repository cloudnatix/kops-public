...
func ListResourcesAzure(cloud azure.AzureCloud, clusterName, resourceGroupName string) (map[string]*resources.Resource, error) {
	g := resourceGetter{
		cloud:             cloud,
...
func (g *resourceGetter) listVMScaleSetsAndRoleAssignments(ctx context.Context) ([]*resources.Resource, error) {
	...
	for i := range vmsses {
		...
		r, err := g.toVMScaleSetResource(vmss)
		if err != nil {
			return nil, err
		}
		rs = append(rs, r)
...
func (g *resourceGetter) toVMScaleSetResource(vmss *compute.VirtualMachineScaleSet) (*resources.Resource, error) {
	...
	if disks := vmss.VirtualMachineProfile.StorageProfile.DataDisks; disks != nil {
		for _, d := range *disks {
...
func (g *resourceGetter) isOwnedByCluster(tags map[string]*string) bool {
	for k, v := range tags {
		if k == azure.TagClusterName && *v == g.clusterName {
			return true
		}
	}
	return false
