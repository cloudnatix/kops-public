...
func TestListResourcesAzure(t *testing.T) {
	...
	e := map[string]*resourceDigest{
		toKey(typeResourceGroup, rgName): &resourceDigest{
			rtype: typeResourceGroup,
			name:  rgName,
		},
		toKey(typeVirtualNetwork, vnetName): &resourceDigest{
			rtype:  typeVirtualNetwork,
			name:   vnetName,
			blocks: []string{toKey(typeResourceGroup, rgName)},
		},
		toKey(typeSubnet, subnetName): &resourceDigest{
			rtype: typeSubnet,
			name:  subnetName,
			blocks: []string{
		...
		toKey(typeRouteTable, rtName): &resourceDigest{
			rtype:  typeRouteTable,
			name:   rtName,
			blocks: []string{toKey(typeResourceGroup, rgName)},
		},
		toKey(typeVMScaleSet, vmssName): &resourceDigest{
		...
		toKey(typeDisk, diskName): &resourceDigest{
			rtype:  typeDisk,
			name:   diskName,
			blocks: []string{toKey(typeResourceGroup, rgName)},
		},
		toKey(typeRoleAssignment, raName): &resourceDigest{
