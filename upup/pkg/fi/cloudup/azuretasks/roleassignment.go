...
func (r *RoleAssignment) Find(c *fi.Context) (*RoleAssignment, error) {
	...
	for _, r := range rs {
		// Use Principal ID to find a Role Assignment. We cannot use a name as
		// it is a randomly generated GUID.
	...
	return &RoleAssignment{
		// TODO(kenji): Fix this. This is based on a hidden
		// assumption that the task name is same as VM Scale
		// Set's name, which is implemented in pkg/model/azuremodel/vmscaleset.go).
		Name:      foundVMSS.Name,
