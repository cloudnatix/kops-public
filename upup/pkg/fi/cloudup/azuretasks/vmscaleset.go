...
func (s *VMScaleSet) RenderAzure(t *azure.AzureAPITarget, a, e, changes *VMScaleSet) error {
	if a == nil {
		return createNewVMScaleSet(t, e)
	}
	// TODO(kenji): Compare a(ctual) and e(xpected) state and update VM Scale Set.
	return errors.New("updating VM Scale Set is not yet implemented")
}

func createNewVMScaleSet(t *azure.AzureAPITarget, e *VMScaleSet) error {
	klog.Infof("Creating a new VM Scale Set with name: %s", fi.StringValue(e.Name))
	name := *e.Name

	var customData *string
	if e.CustomData != nil {
		d, err := e.CustomData.AsBytes()
		if err != nil {
